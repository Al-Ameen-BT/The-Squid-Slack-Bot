...
```bash
sudo mkdir -p /opt/slack-squid-proxy
sudo cp -r ./* /opt/slack-squid-proxy/
cd /opt/slack-squid-proxy
...
Ensure the `squid` user actually owns the ACL mapping folders it writes into dynamically:
```bash
